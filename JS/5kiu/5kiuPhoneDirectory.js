function phone(strng, num) {
    let contact = strng.split("\n").filter(e => e.match(num));
    if (contact.length > 1) return `Error => Too many people: ${num}`;
    if (contact.length === 0) return `Error => Not found: ${num}`;
    num = contact[0].match(/\d+-\d{3}-\d{3}-\d{4}/)[0];
    const name = contact[0].match(/<.*>/)[0];
    const address = contact[0]
        .replace(/[?!:+/;,$*]/g, "")
        .replace(num, "")
        .replace(name, "")
        .trim();
    return `Phone => ${num}, Name => ${name.replace(/<|>/gm, "")}, Address => ${address.replace(/_|\s\s+/g," ").trim()}`;
}
